...
# ============================================================================
# TEST 2: MULTIFLEET WITH 1 FLEET (catch) (SHOULD MATCH SINGLE FLEET)
# ============================================================================
...
compare_metrics <- function(result1, result2, name1, name2, tolerance = 1e-10) {
  ...
  # Compare total F
  f_diff <- max(abs(result1$dynamics$Ftotal - result2$dynamics$Ftotal))
...
for(i in 1:length(all_results)) {
  final_sb <- sum(all_results[[i]]$dynamics$SB[15, , ])  # Year 15, all iterations, all areas
