#MP5: Index slope (1 or multiple indices) + length indicators (none, one or multiple length indicators)

# Similar to GB_SLOPE (Geromont and Butterworth Index Slope) but uses standardized slope and includes biomass safeguards
...
#   This MP combines:
#   1. Index slope (ISLOPE method) - standardized abundance trend
#   2. Length indicators - size composition metrics
#   3. Index safeguard - biomass protection trigger


#global storage for Newton-Raphson diagnostics
nr_diagnostics_multifleet <<- data.frame()

multifleet_islope_length_MP   <- function(phase, dataObject) {
  ...
  index_config <- list(
    ...
    #2. number of recent observations for slope calculation
    n_years_for_slope = 5,  # window for linear regression (typically 5 years)

    #3. slope threshold (prevents action on negligible trends)
    # This is the minimum standardized slope required to trigger TAC adjustment
    # If standardized_slope < slope_threshold, set slope = 0 (no adjustment)

    slope_threshold = 0.05,  # 5% per year threshold- prevents TAC changes from noise in survey data


    #4. gamma parameter (gain parameter: controling reponse strength to standardized slope)
    # gamma = 0.5: Conservative response
    # gamma = 1.0: Proportional response (default)
    # gamma = 2.0: Aggressive response
    gamma = 1.0,


    #5. how to combine multiple indices
    # "weighted_average": Use weights to combin- Higher weight = more influence on final slope
    # "multiplicative": Multiply (1+slope1) × (1+slope2)
    # "minimum": Using the most pessimistic slope (conservative)


    index_combination_method = "weighted_average",  # Options: "weighted_average", "multiplicative", "minimum"

    #6. Thr weigthing approach
    index_weights = c(1.0,0.8),  # match length of survey_index_names

    #7. Enable/disable index
    use_index_slope = TRUE,    # FALSE: set slope_multiplier = 1.0 (no effect)

    #8. enable the use of index safeguard (ISLOPE-specific)
    #   based on index level, not stock biomass
    #   reduces TAC when current index falls below trigger
    use_index_safeguard  = TRUE,

    breaker_lookback_years = 5,  # use last 5 years to calculate I_max

    #9. safeguard trigger point (proportion of maximum observed index)
    #   when current index < trigger_proportion × I_max, apply safeguard
    #   I_max = highest index value observed across all years

    index_trigger_proportion = 0.4,  # 40% of I_max

    #10. safeguard multiplier (TAC reduction when below trigger)
    #    TAC reduction when safeguard is triggered
    #    applied independently of slope multiplier
    index_safeguard_multiplier = 0.8,  # 20% reduction when triggered

    #11. stability clauses for slope-based adjustments
    #    prevents extreme year-to-year TAC variability
    #    apply stability constraints only when safeguard not triggered
  ...
  #Length indicator configuration (this can be replaced by other L indicators - I just want to illustrate an example)

  length_config <- list(

    #1. enable/disable all indicators
    use_length_indicators = TRUE,

    #2. enable/disable each indicator with TRUE or FALSE
    ...
    use_L_mean_ratio = TRUE,     #temporal trend

    #3. weights for combining indicators (only for enabled indicators)
    weights = c(
      L_mean_L_mat = 0.35,
      #L_mean_L_opt = 0.25,
      Pmature = 0.35,
    ...
    #4. target values (what we want each indicator to achieve)
    targets = list(
    ...
    #5. penalty multipliers (TAC reduction when below target)
    ...
    #6. multipliers for TAC increase when above a target)
    ...
    #7. combination method for multiple length indicators
    ...
    #9. maximum TAC increase based on length indicators
    max_tac_increase = 1.2,  # 20% maximum increase from length indicators

    length_lookback_years = 5, #for L_mean_ratio
  ...
  #1.precautionary multiplier
  precautionary_m <- 0.95     # 5% precautionary reduction

  #2. Fleet allocation: How should TAC be split among fleets?
  ...
  #3. Number of recent years for calculating fleet proportions (automatic mode)
  ...
  #5. number of years to average for initial TAC calculation
  ...
  if(phase == 2) {
    ...
    # Steps:
    #   1. Determine previous year's TAC
    #   2. Calculate index slope adjustment
    #   3. Calculate length indicator adjustment
    #   4. Apply index safeguard if needed
    #   5. Calculate preliminary TAC
    #   6. Apply stability constraints
    #   7. Allocate TAC among fleets
    #   8. Allocate fleet TAC among areas
    ...
    cat(sprintf("\n=== MULTI-SLOPE-INDEX-LENGTH MP: Year %d, Iteration %d ===\n", real_year, k))
    ...
    #Step 2: calculate standardized index slope (ISLOPE method)

    slope_multiplier <- 1.0      #final TAC adjustment (default no change)
    combined_slope <- 0          #weighted average of standardized slopes
    individual_slopes <- c()     #raw slopes (units: index change per year)
    individual_std_slopes <- c() #standardized slopes (proportional)
    index_details <- list()      #stores metadata for each index


    if(index_config$use_index_slope) {
      cat(sprintf("\n=== processing %d Index/Indices ===\n",
                  length(index_config$survey_index_names)))

      standardized_slopes <- c()  # how many indices will be used

      #calculate standardized slope for each index
      for(idx in 1:length(index_config$survey_index_names)) {
        ...
        index_data  <- decisionData[decisionData$k == k &
                                      decisionData$j <= obs_data_year,
                                    c("j", index_name)]
        # remove NA values (years without survey observations)
        index_data  <- index_data [!is.na(index_data [[index_name]]), ]

        #validating enough survey observations for trend calculation
        min_required <- index_config$n_years_for_slope

        if(nrow(index_data ) < min_required) {
          stop("Index '", index_name, "': Insufficient data. Need ", min_required,
               " observations, found ", nrow(index_data ))
        }

        # calculate recent window
        recent_data <- tail(index_data, index_config$n_years_for_slope)

        # fit linear regression: Index ~ Year
        years_numeric <- recent_data$j
        index_values <- recent_data[[index_name]]
        lm_fit <- lm(index_values ~ years_numeric)
        raw_slope <- coef(lm_fit)[2]

        # ISLOPE: standardize slope by mean index
        mean_index <- mean(index_values, na.rm = TRUE)
        standardized_slope <- raw_slope / mean_index # standardization: scale-independent, so it works regardless of index units. We can combine diff index types

        # apply slope threshold (If standardized_slope < 0.05, set to 0)
        if(abs(standardized_slope) < index_config$slope_threshold) {
          standardized_slope <- 0
          cat(sprintf("Index %d (%s): Slope below threshold, set to 0\n",
                      idx, index_name))
        }
        #store slopes in vectors
        standardized_slopes <- c(standardized_slopes, standardized_slope)
        individual_slopes <- c(individual_slopes, raw_slope)
        individual_std_slopes <- c(individual_std_slopes, standardized_slope)
        ...
        index_details[[index_name]] <- list(
          ...
          years_used = length(years_numeric),
          threshold_applied = abs(raw_slope / mean_index) < index_config$slope_threshold
        )

        cat(sprintf("Index %d (%s):\n", idx, index_name))
        cat(sprintf("  Years used: %d (j=%d to j=%d)\n",
                    length(years_numeric), min(years_numeric), max(years_numeric)))
        cat(sprintf("  Mean index: %.4f\n", mean_index))
        cat(sprintf("  Raw slope: %.6f\n", raw_slope))
        cat(sprintf("  Standardized slope: %.6f (%.2f%% per year)\n",
                    standardized_slope, standardized_slope * 100))
      }


      #combining multiple indices

      if(length(standardized_slopes) > 1) {
        cat(sprintf("\nCombining %d indices using '%s' method:\n",
                    length(standardized_slopes), index_config$index_combination_method))

        if(index_config$index_combination_method == "weighted_average") {
          # Validate weights
          if(length(index_config$index_weights) != length(standardized_slopes)) {
            stop("index_weights length must match number of indices")
          }
          weights <- index_config$index_weights / sum(index_config$index_weights)
          combined_slope <- sum(standardized_slopes * weights)
          # e.g., combined_slope = 0.1717 × 0.556 + (-0.0523) × 0.444
          cat(sprintf("  Weights: %s\n", paste(sprintf("%.2f", weights), collapse=", ")))
          cat(sprintf("  Combined standardized slope = %.6f\n", combined_slope))

        } else if(index_config$index_combination_method == "multiplicative") {
          #both indices must be positive for positive combined slope
          # e.g.:  Index 1: slope = +0.10 = (1 + 0.10) = 1.10
          #        Index 2: slope = +0.08 = (1 + 0.08) = 1.08
          # combined effect: 1.10 × 1.08 = 1.188
          # converting back to slope for scaling consistency
          # 1.188 - 1 = 0.188 (combined slope)

          combined_slope <- prod(1 + standardized_slopes) - 1
          cat(sprintf("  Product-based slope = %.6f\n", combined_slope))

        } else if(index_config$index_combination_method == "minimum") {
          combined_slope <- min(standardized_slopes)
          cat(sprintf("  Minimum slope = %.6f\n", combined_slope))
        }
      } else {
        combined_slope <- standardized_slopes[1]
      }


      # calculate slope multiplier using gamma
      # ISLOPE formula: convert standardized slope to TAC multiplier
      slope_multiplier <- 1 + (index_config$gamma * combined_slope)
      #Start with 1.0 (no change baseline)
      #Add: gamma × combined_slope
      #gamma = gain parameter

      #e.g.,: aggressive gamma = 2.0
      #combined_slope = 0.10 (10% per year increase)
      #slope_multiplier = 1 + (2.0 × 0.10) = 1.20
      #interpretation: 20% increase for 10% slope


      cat(sprintf("\nSlope-based adjustment:\n"))
      cat(sprintf("  Combined standardized slope: %.6f (%.2f%% per year)\n",
                  combined_slope, combined_slope * 100))
      cat(sprintf("  Gamma (gain parameter): %.2f\n", index_config$gamma))
      cat(sprintf("  Slope multiplier (before constraints): %.4f ", slope_multiplier))

      if(slope_multiplier > 1) {
        cat(sprintf("(+%.1f%% TAC increase)\n", (slope_multiplier - 1) * 100))
      } else if(slope_multiplier < 1) {
        cat(sprintf("(%.1f%% TAC decrease)\n", (1 - slope_multiplier) * 100))
      } else {
        cat("(No change)\n")
      }
    } else {
      # use_index_slope = FALSE
      cat("\nIndex slope: DISABLED (slope_multiplier = 1.0)\n")
    }



    #Step 3: length indicator adjustmet
    ...
    if(length_config$use_length_indicators) {
        ...
        calculate_length_indicators <- function(lh, length_config, decisionData,
          ...
          if(length_config$use_Pmature) {
            if(is_multifleet) {
              ...
              #catch-weighted Pmature (use same fleet_catch_props from above)
     ...
     catchB_by_fleet = catchB_by_fleet  # true catches for weighting
        ...
        combine_length_indicators <- function(indicators, length_config) {
          ...
          adjustments <- sapply(indicator_names, function(name) {
            ...
            if(observed < target) {
              ...
              adjustment <- min(adjustment, length_config$max_tac_increase)  # Cap at 130% of TAC (30% increase maximum)
            }
        ...
        if(!is.null(length_combination$weights_used)) {
          cat(sprintf("  Weights: %s\n",
                      paste(sprintf("%s=%.2f",
                                    names(length_combination$weights_used),
                                    length_combination$weights_used),
                            collapse=", ")))
        }
    ...
    # step 3.5: Index safeguard (rolling I_max protection)
    index_safeguard <- 1.0

    if(index_config$use_index_safeguard) {
      cat(sprintf("\n=== Index-based Safeguard (Rolling I_max) ===\n"))

      #get configuration parameter
      breaker_lookback_years <- index_config$breaker_lookback_years

      #get current index value
      current_indices <- sapply(index_config$survey_index_names, function(index_name) {
        recent_vals <- decisionData[[index_name]][
          decisionData$k == k & decisionData$j <= obs_data_year]
        recent_vals <- recent_vals[!is.na(recent_vals)]
        if(length(recent_vals) > 0) tail(recent_vals, 1) else NA
      })

      current_index <- mean(current_indices, na.rm = TRUE)

      #determine the lookback window for I_max calculation
      start_year_lookback <- max(2, obs_data_year - breaker_lookback_years)
      end_year_lookback <- obs_data_year

      cat(sprintf(" rolling window for I_max: years %d-%d (j=%d-%d)\n",
                  start_year_lookback - 1, end_year_lookback - 1,
                  start_year_lookback, end_year_lookback))
      cat(sprintf(" lookback period: %d years\n", breaker_lookback_years))

      # extract indices from rolling window only
      rolling_indices <- sapply(index_config$survey_index_names, function(index_name) {
        vals <- decisionData[[index_name]][
          decisionData$k == k &
            decisionData$j >= start_year_lookback &
            decisionData$j <= end_year_lookback]
        vals[!is.na(vals)]
      })

      # I_max = maximum observed in the rollimg window
      I_max <- max(unlist(rolling_indices), na.rm = TRUE)

      # calculate trigger as proportion of recent I_max
      I_trigger <- I_max * index_config$index_trigger_proportion

      cat(sprintf("  Current index: %.4f\n", current_index))
      cat(sprintf("  I_max (rolling %d years): %.4f\n",
                  breaker_lookback_years, I_max))
      cat(sprintf("  I_trigger (%.0f%% of I_max): %.4f\n",
                  index_config$index_trigger_proportion * 100, I_trigger))
      cat(sprintf("  Ratio (current/I_max): %.3f\n", current_index / I_max))

      #if index falls below trigger, apply safeguard
      if(current_index < I_trigger) {
        index_safeguard <- index_config$index_safeguard_multiplier
        cat(sprintf("\n SAFEGUARD TRIGGERED: Current < Trigger\n"))
        cat(sprintf("  Current (%.4f) < I_trigger (%.4f)\n",
                    current_index, I_trigger))
        cat(sprintf("  Applying %.0f%% reduction (multiplier = %.2f)\n",
                    (1 - index_safeguard) * 100, index_safeguard))
      } else {
        cat(sprintf(" Index above trigger: No safeguard reduction\n"))
      }

    } else {
      cat("\nIndex safeguard: DISABLED (index_safeguard = 1.0)\n")
    }


    #Step 4: Apply stability constraints to slope multiplier

    if(index_config$use_index_slope) {
      original_slope_multiplier <- slope_multiplier

      if(index_config$stability_only_when_healthy && index_safeguard  < 1.0) {
        # Stock below trigger - no stability constraints on slope
        cat(sprintf("\nSlope stability: NOT APPLIED (index safeguard triggered, index_safeguard=%.2f)\n", index_safeguard))
        cat(sprintf("  Slope multiplier: %.4f (unconstrained)\n", slope_multiplier))


      } else {
        # Apply stability constraints to slope multiplier
        max_slope_increase <- 1 + index_config$max_increase
        max_slope_decrease <- 1 - index_config$max_decrease

        slope_multiplier_before <- slope_multiplier
        slope_multiplier <- max(max_slope_decrease, min(slope_multiplier, max_slope_increase))

        cat(sprintf("\nSlope stability constraints:\n"))
        cat(sprintf("  Allowed range: %.4f to %.4f (-%.0f%% to +%.0f%%)\n",
                    max_slope_decrease, max_slope_increase,
                    index_config$max_decrease * 100, index_config$max_increase * 100))
        cat(sprintf("  Original slope multiplier: %.4f ", slope_multiplier_before))

        if(slope_multiplier != slope_multiplier_before) {
          if(slope_multiplier == max_slope_increase) {
            cat(sprintf("CAPPED at %.4f (+%.0f%%)\n",
                        slope_multiplier, index_config$max_increase * 100))
          } else {
            cat(sprintf("FLOORED at %.4f (-%.0f%%)\n",
                        slope_multiplier, index_config$max_decrease * 100))
          }
        } else {
          cat("(no constraint applied)\n")
        }
        cat(sprintf("  Final slope multiplier: %.4f\n", slope_multiplier))
      }
    }


    #Step 5: Calculate Preliminary TAC
    TAC_preliminary <- TAC_previous * slope_multiplier  * index_safeguard * f_length * precautionary_m

    cat(sprintf("\nPreliminary TAC calculation:\n"))
    cat(sprintf("  TAC_preliminary = TAC_previous × slope_mult × index_safeguard × f_length × m\n"))
    cat(sprintf("  TAC_preliminary = %.2f × %.4f × %.2f × %.3f × %.2f\n",
                TAC_previous, slope_multiplier, index_safeguard, f_length, precautionary_m))
    cat(sprintf("  TAC_preliminary = %.2f\n", TAC_preliminary))

    #Step 6: final TAC stability clause
    ...
    if(index_config$stability_only_when_healthy && index_safeguard  < 1.0) {
      ...
      stability_applied <- sprintf("NO (safeguard triggered, index_safeguard=%.2f)", index_safeguard)
      ...
      TAC_max <- TAC_previous * (1 + index_config$max_increase)
      TAC_min <- TAC_previous * (1 - index_config$max_decrease)
      TAC_final <- max(TAC_min, min(TAC_preliminary, TAC_max))
      stability_applied <- sprintf("YES (+%.0f%%/-%.0f%%)",
                                   index_config$max_increase*100,
                                   index_config$max_decrease*100)


      cat(sprintf("\nFinal TAC stability clause: YES\n"))
      cat(sprintf("  TAC_min (-%.0f%%) = %.2f\n", index_config$max_decrease*100, TAC_min))
      cat(sprintf("  TAC_max (+%.0f%%) = %.2f\n", index_config$max_increase*100, TAC_max))
    ...
    for(f in 1:nfleets) {
      ...
      for(m in 1:areas) {
        ...
        TAC_decisions <- rbind(TAC_decisions, data.frame(
          ...
          combined_std_slope = combined_slope,
          gamma = index_config$gamma,
          n_indices_used = length(index_config$survey_index_names),
          index_combination_method = index_config$index_combination_method,

          #Index safeguard
          index_safeguard = index_safeguard,
          index_safeguard_triggered = if(index_config$use_index_safeguard) index_safeguard < 1.0 else NA,
          ...
          L_mean_ref_years_start = ifelse(!is.null(length_results$diagnostics$L_mean_ref_years),
                                          length_results$diagnostics$L_mean_ref_years[1], NA),
          L_mean_ref_years_end = ifelse(!is.null(length_results$diagnostics$L_mean_ref_years),
                                        length_results$diagnostics$L_mean_ref_years[2], NA),
          ...
          safeguard_enabled = index_config$use_index_safeguard,
...
result_multi_indexratio_lengthV2<- runProjection(
  ...
  TimeAreaObj = ta,
  StrategyObj = strategy_indexslope_length,
  ...
  wd = getwd(),
  fileName = "test_multifleet_islope_length_MP",
  seed = test_seed,
  doPlot = FALSE,
  doDiagnostic = FALSE,
  customToCluster = "multifleet_islope_length_MP"
)
cat(" multiple Index slope - lenght MP simulation completed\n")

# all debugging code reviewed!! and passed!!

result_multi_indexslope_length <- readProjection(getwd(), "test_multifleet_islope_length_MP")


# Population outputs
result_multi_indexslope_length$dynamics$multifleet$actual_catch_proportions
result_multi_indexslope_length$dynamics$SB
result_multi_indexslope_length$dynamics$recN
result_multi_indexslope_length$dynamics$SPR

# obs model outputs (new structure data frame)
result_multi_indexslope_length$HCR$decisionData
result_multi_indexslope_length$HCR$decisionData$IDX_Survey_1
result_multi_indexslope_length$HCR$decisionLocal

result_multi_indexslope_length$HCR$decisionData$fleet_1_observed_catch_area_1
result_multi_indexslope_length$HCR$decisionData$fleet_1_observed_catch_area_2
result_multi_indexslope_length$HCR$decisionData$fleet_2_observed_catch_area_1
result_multi_indexslope_length$HCR$decisionData$fleet_2_observed_catch_area_2


result_multi_indexslope_length$HCR$decisionAnnual$TAC
result_multi_indexslope_length$HCR$decisionAnnual
...
plot_SB(result_multi_indexslope_length)
plot_SB(result_multi_indexslope_length, areas=1)
plot_SB(result_multi_indexslope_length, areas=2)


plot_catchB(result_multi_indexslope_length)
plot_catchB(result_multi_indexslope_length,areas=1)
plot_catchB(result_multi_indexslope_length,areas=2)

plot_catchN(result_multi_indexslope_length)
plot_catchN(result_multi_indexslope_length, areas=1)
plot_catchN(result_multi_indexslope_length, areas=2)

plot_discB(result_multi_indexslope_length)
plot_discB(result_multi_indexslope_length,areas=1)
plot_discB(result_multi_indexslope_length,areas=2)

plot_discN(result_multi_indexslope_length)
plot_discN(result_multi_indexslope_length,areas=1)
plot_discN(result_multi_indexslope_length,areas=2)

plot_catchB_multi(result_multi_indexslope_length)
plot_catchB_multi(result_multi_indexslope_length, areas=1)
plot_catchB_multi(result_multi_indexslope_length, areas=2)

plot_catchN_multi(result_multi_indexslope_length,show_individual = TRUE)
plot_catchN_multi(result_multi_indexslope_length, areas=1)
plot_catchN_multi(result_multi_indexslope_length, areas=2)

plot_SPR(result_multi_indexslope_length)
plot_recN(result_multi_indexslope_length)


#plot obs models (indices)
plot_survey_indices(result_multi_indexslope_length)
plot_cpue_indices(result_multi_indexslope_length)

plot_all_indices(result_multi_indexslope_length)

#plot individual indices
plot_indices(result_multi_indexslope_length,
             index_pattern = "IDX_Survey_1",
             show_individual = TRUE,
             title = "FI survey")

# Total catch across all areas
plot_catch_observations_both(result_multi_indexslope_length,areas = c(1,2),show_individual=TRUE)
plot_catch_observations_both(result_multi_indexslope_length, areas = 1)
plot_catch_observations_both(result_multi_indexslope_length, areas = 2)


# plot LC obs models
# NEW: Area-specific functions (median across iterations are dispayed)
plot_fishery_length_comp(result_multi_indexslope_length,show_individual = TRUE)
plot_fishery_length_comp(result_multi_indexslope_length, areas=1,show_individual = TRUE)
plot_fishery_length_comp(result_multi_indexslope_length, areas=2,show_individual = TRUE)

# plot_survey_length_comp(result_multi_indexslope_length,show_individual = TRUE)
# plot_survey_length_comp(result_multi_indexslope_length, areas=1,show_individual = TRUE)
# plot_survey_length_comp(result_multi_indexslope_length, areas=2,show_individual = TRUE)



# NEW: Custom filtering for fleets and areas
plot_length_composition_by_area(result_multi_indexslope_length,
...
plot_Ftotal_multi(result_multi_indexslope_length,areas=c(1,2))
plot_Ftotal_multi(result_multi_indexslope_length,areas=c(1))
plot_Ftotal_multi(result_multi_indexslope_length,areas=c(2))
...
plot_TAC(result_multi_indexslope_length, areas = "all",show_fleets = TRUE)  # All areas in one plot
#plot_TAC_by_fleet(result_indexratio)
plot_TAC(result_multi_indexslope_length, areas = c(1), show_fleets = TRUE)
plot_TAC(result_multi_indexslope_length, areas = c(2), show_fleets = TRUE)
...
plot_catch_observations_both(result_multi_indexslope_length, areas = c(1, 2))
#combined catch obs:
plot_catch_observations_both(result_multi_indexslope_length,areas = c(1, 2), show_individual = TRUE)
...
catchB <- result_multi_indexslope_length$dynamics$catchB
TAC_decisions <- result_multi_indexslope_length$HCR$decisionAnnual
  ...
  for(iter in 1:3) {
    for(area in 1:2) {
      ...
      for(fleet in 1:nfleets) {
        ...
        realized_fleet <- result_multi_indexslope_length$dynamics$multifleet$catchB_by_fleet[yr, iter, area, fleet]
...
lh <- LHwrapper(result_multi_indexslope_length$LifeHistoryObj, result_multi_indexslope_length$TimeAreaObj)
...
for(age in test_ages) {
  ...
  fleet1_sel <- selWrapper(lh, result_multi_indexslope_length$TimeAreaObj,
                           FisheryObj = result_multi_indexslope_length$MultifleetObj@fleet_selectivity_proj_list[[1]][[1]],
                           doPlot = FALSE)

  fleet2_sel <- selWrapper(lh, result_multi_indexslope_length$TimeAreaObj,
                           FisheryObj = result_multi_indexslope_length$MultifleetObj@fleet_selectivity_proj_list[[1]][[2]],
                           doPlot = FALSE)
