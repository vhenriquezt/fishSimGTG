...
# multifleet MP: Index slope + length indicators
# uses standardized linear regression of abundance indices to adjust TAC
...
multifleet_islope_length_MP   <- function(phase, dataObject) {
  ...
  index_config <- list(
   ...
   #1. Biomass safeguard (stock-based, not index-based)
   use_biomass_safeguard <- TRUE
   trigger_proportion <- 0.20  # 20% of B0
  ...
  if(phase == 2) {
    ...
    index_safeguard <- 1.0       #initialize index-based safeguard multiplier
    ...
    if(index_config$use_index_slope) {
      ...
      # ISLOPE: Index-based safeguard
      index_safeguard <- 1.0
      if(index_config$use_index_safeguard) {
        cat(sprintf("\n=== Index-based Safeguard (ISLOPE) ===\n"))
        ...
        # get maximum observed index
        all_indices <- sapply(index_config$survey_index_names, function(index_name) {
          vals <- decisionData[[index_name]][decisionData$k == k]
          vals[!is.na(vals)]
        })
        I_max <- max(unlist(all_indices), na.rm = TRUE) #maximum ever observed

        # calculate trigger
        I_trigger <- I_max * index_config$index_trigger_proportion  #I_max × 0.4

        cat(sprintf("  Current index: %.4f\n", current_index))
        cat(sprintf("  I_max: %.4f\n", I_max))
        cat(sprintf("  I_trigger (%.0f%% of I_max): %.4f\n",
                    index_config$index_trigger_proportion * 100, I_trigger))

        #If index falls below 40% of historical maximum, apply safeguard
        if(current_index < I_trigger) {
          index_safeguard <- index_config$index_safeguard_multiplier
          cat(sprintf("  SAFEGUARD TRIGGERED: Current < Trigger\n"))
      ...
      index_safeguard <- 1.0
      cat("\nIndex slope: DISABLED (multiplier = 1.0)\n")
    }
    ...
    #Step 6: Apply stability constraints to slope multiplier

    if(index_config$use_index_slope) {
      original_slope_multiplier <- slope_multiplier

      if(index_config$stability_only_when_healthy && b < 1.0) {
        # Stock below trigger - no stability constraints on slope
        cat(sprintf("\nSlope stability: NOT APPLIED (safeguard triggered, b=%.2f)\n", b))
    ...
    TAC_preliminary <- TAC_previous * slope_multiplier  * index_safeguard * f_length * b * precautionary_m

    cat(sprintf("\nPreliminary TAC calculation:\n"))
    cat(sprintf("  TAC_preliminary = TAC_previous × slope_mult × index_safeguard × f_length × b × m\n"))
    cat(sprintf("  TAC_preliminary = %.2f × %.4f × %.2f × %.3f × %.3f × %.2f\n",
                TAC_previous, slope_multiplier, index_safeguard, f_length, b, precautionary_m))
    cat(sprintf("  TAC_preliminary = %.2f\n", TAC_preliminary))

    #Step 8: final TAC stability clause
