# ============================================================================
# TEST: KEEP f CONSTANT AND CHANGE SEL IN PROJECTIONS
# ============================================================================
...
ta@historicalEffort <- matrix(4.5, nrow = 10, ncol = 2)

# Stochastic
stochastic_obj <- new("Stochastic")
stochastic_obj@historicalBio <- c(0.55, 0.56)
stochastic_obj@Steep <- c(0.65, 0.66)
...
proj_fishery_area1@vulParams<-c(12.2, 0.1)
...
proj_fishery_area2@vulParams<-c(12.2, 0.1)
...
singleCompMP <- function(phase, dataObject) {
  ...
  if(phase == 3) {
    ...
    area <- 1:areas
    Flocal <- rep(0.2, areas)  # Conservative F
...
fleet2_sel_hist@vulParams <- c(10.2, 0.1)  # Different selectivity
...
fleet3_sel_hist@vulParams <- c(10.2, 0.1)  # Different selectivity
...
fleet1_sel_proj@vulParams <- c(12.2, 0.11)
...
fleet2_sel_proj@vulParams <- c(12.2, 0.11)
...
fleet3_sel_proj@vulParams <- c(12.2, 0.11)
  ...
  if(phase == 3) {
    ...
    for(m in 1:areas) {
      for(f in 1:nfleets) {
        ...
        Flocal <- rbind(Flocal, c(j, k, m, f, 0.08))  # Conservative F = 0.05
      }
    }
