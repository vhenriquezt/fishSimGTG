#MP3: Index ratio + length indicators
...
multifleet_indexratio_length_MP   <- function(phase, dataObject) {
  ...
  #3. Length indicator configuration (this can be replaced by other L indicators - I just want to illustrate an example)
  ...
  length_config <- list(
    ...
    use_Pmature = FALSE,         # Proportion mature in catch
  ...
  #4. enable the use of biomass safeguard based on unfished spawning biomass
  use_biomass_safeguard <- TRUE

  #5. trigger point as proportion of B0 (e.g., Sb/B0)
  trigger_proportion <- 0.20  # Trigger at 20% of unfished biomass

  #6. precautionary multiplier
  ...
  #8. How should TAC be split among fleets?
  ...
  #9. Number of recent years for calculating fleet proportions (automatic mode)
  ...
  #11. number of years to average for initial TAC calculation
  ...
  if(phase == 2) {
    ...
    cat(sprintf("Data timing: all obs data from previous year\n"))
    ...
    if(nrow(survey_data) < min_required) {
      stop("Insufficient survey data. Need at least ", min_required,
...
strategy_indexratio_length @title <- "Index Ratio and length MP with Biomass Safeguard"
strategy_indexratio_length @projectionYears <- 5
strategy_indexratio_length @projectionName <- "multifleet_indexratio_length_MP"
...
result_multi_indexratio_length<- runProjection(
  ...
  wd = getwd(),
  fileName = "test_indexratio_length_MP",
  seed = test_seed,
  doPlot = FALSE,
  doDiagnostic = FALSE,
  customToCluster = "multifleet_indexratio_length_MP"
)
cat("Index Ratio - lenght MP simulation completed\n")

# all debugging code reviewed!! and passed!!

result_multi_indexratio_length <- readProjection(getwd(), "test_indexratio_length_MP")


# Population outputs
result_multi_indexratio_length$dynamics$multifleet$actual_catch_proportions
result_multi_indexratio_length$dynamics$SB
result_multi_indexratio_length$dynamics$recN
result_multi_indexratio_length$dynamics$SPR

# obs model outputs (new structure data frame)
result_multi_indexratio_length$HCR$decisionData
result_multi_indexratio_length$HCR$decisionData$IDX_Survey_1
result_multi_indexratio_length$HCR$decisionLocal

result_multi_indexratio_length$HCR$decisionData$fleet_1_observed_catch_area_1
result_multi_indexratio_length$HCR$decisionData$fleet_1_observed_catch_area_2
result_multi_indexratio_length$HCR$decisionData$fleet_2_observed_catch_area_1
result_multi_indexratio_length$HCR$decisionData$fleet_2_observed_catch_area_2


result_multi_indexratio_length$HCR$decisionAnnual$TAC
result_multi_indexratio_length$HCR$decisionAnnual
...
plot_SB(result_multi_indexratio_length)
plot_SB(result_multi_indexratio_length, areas=1)
plot_SB(result_multi_indexratio_length, areas=2)


plot_catchB(result_multi_indexratio_length)
plot_catchB(result_multi_indexratio_length,areas=1)
plot_catchB(result_multi_indexratio_length,areas=2)

plot_catchN(result_multi_indexratio_length)
plot_catchN(result_multi_indexratio_length, areas=1)
plot_catchN(result_multi_indexratio_length, areas=2)

plot_discB(result_multi_indexratio_length)
plot_discB(result_multi_indexratio_length,areas=1)
plot_discB(result_multi_indexratio_length,areas=2)

plot_discN(result_multi_indexratio_length)
plot_discN(result_multi_indexratio_length,areas=1)
plot_discN(result_multi_indexratio_length,areas=2)

plot_catchB_multi(result_multi_indexratio_length)
plot_catchB_multi(result_multi_indexratio_length, areas=1)
plot_catchB_multi(result_multi_indexratio_length, areas=2)

plot_catchN_multi(result_multi_indexratio_length,show_individual = TRUE)
plot_catchN_multi(result_multi_indexratio_length, areas=1)
plot_catchN_multi(result_multi_indexratio_length, areas=2)

plot_SPR(result_multi_indexratio_length)
plot_recN(result_multi_indexratio_length)


#plot obs models (indices)
plot_survey_indices(result_multi_indexratio_length)
plot_cpue_indices(result_multi_indexratio_length)

plot_all_indices(result_multi_indexratio_length)

#plot individual indices
plot_indices(result_multi_indexratio_length,
             index_pattern = "IDX_Survey_1",
             show_individual = TRUE,
             title = "FI survey")

# Total catch across all areas
plot_catch_observations_both(result_multi_indexratio_length,areas = c(1,2),show_individual=TRUE)
plot_catch_observations_both(result_multi_indexratio_length, areas = 1)
plot_catch_observations_both(result_multi_indexratio_length, areas = 2)


# plot LC obs models
# NEW: Area-specific functions (median across iterations are dispayed)
plot_fishery_length_comp(result_multi_indexratio_length,show_individual = TRUE)
plot_fishery_length_comp(result_multi_indexratio_length, areas=1,show_individual = TRUE)
plot_fishery_length_comp(result_multi_indexratio_length, areas=2,show_individual = TRUE)

# plot_survey_length_comp(result_multi_indexratio_length,show_individual = TRUE)
# plot_survey_length_comp(result_multi_indexratio_length, areas=1,show_individual = TRUE)
# plot_survey_length_comp(result_multi_indexratio_length, areas=2,show_individual = TRUE)



# NEW: Custom filtering for fleets and areas
plot_length_composition_by_area(result_multi_indexratio_length,
...
plot_Ftotal_multi(result_multi_indexratio_length,areas=c(1,2))
plot_Ftotal_multi(result_multi_indexratio_length,areas=c(1))
plot_Ftotal_multi(result_multi_indexratio_length,areas=c(2))
...
plot_TAC(result_multi_indexratio_length, areas = "all",show_fleets = TRUE)  # All areas in one plot
#plot_TAC_by_fleet(result_indexratio)
plot_TAC(result_multi_indexratio_length, areas = c(1), show_fleets = TRUE)
plot_TAC(result_multi_indexratio_length, areas = c(2), show_fleets = TRUE)
...
plot_catch_observations_both(result_multi_indexratio_length, areas = c(1, 2))
#combined catch obs:
plot_catch_observations_both(result_multi_indexratio_length,areas = c(1, 2), show_individual = TRUE)
...
catchB <- result_multi_indexratio_length$dynamics$catchB
TAC_decisions <- result_multi_indexratio_length$HCR$decisionAnnual
  ...
  for(iter in 1:3) {
    for(area in 1:2) {
      ...
      for(fleet in 1:nfleets) {
        ...
        realized_fleet <- result_multi_indexratio_length$dynamics$multifleet$catchB_by_fleet[yr, iter, area, fleet]
...
lh <- LHwrapper(result_multi_indexratio_length$LifeHistoryObj, result_multi_indexratio_length$TimeAreaObj)
...
for(age in test_ages) {
  ...
  fleet1_sel <- selWrapper(lh, result_multi_indexratio_length$TimeAreaObj,
                           FisheryObj = result_multi_indexratio_length$MultifleetObj@fleet_selectivity_proj_list[[1]][[1]],
                           doPlot = FALSE)

  fleet2_sel <- selWrapper(lh, result_multi_indexratio_length$TimeAreaObj,
                           FisheryObj = result_multi_indexratio_length$MultifleetObj@fleet_selectivity_proj_list[[1]][[2]],
                           doPlot = FALSE)
